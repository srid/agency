...
- `--review-model=<model>`: Model to use for the **hickey+lowy** sub-agent invocations. Pass-through to the harness's delegation tool verbatim — Claude Code accepts bare aliases (`opus`, `sonnet`, `haiku`); opencode requires `provider/modelID` form (e.g. `anthropic/claude-sonnet-4-5`). The user owns whether the value resolves on their harness. When the flag is omitted, the default is harness-specific: `sonnet` on Claude Code (cheap enough to run on every task, and protects weekly-Opus budget); the parent session's model on opencode (since opencode's model namespace is per-user and the session model is what the user already chose for cost). Has no effect under `--minimal` since hickey+lowy is skipped.
...
**Model override.** If the user passed `--review-model=<value>`, pass `model: "<value>"` verbatim in **both** delegation tool calls (`Agent` on Claude Code, `task` on opencode, etc.). The harness is responsible for resolving the value; if it can't, let the harness's error surface — the user owns the choice. If the flag was not passed, the default depends on the delegation tool: on `Agent` (Claude Code), pass `model: "sonnet"` to keep reviews cheap and protect the weekly Opus budget; on `task` (opencode), omit the `model` parameter so the sub-agent inherits the parent session's model — opencode's model namespace is per-user, and the session model is what the user already chose for cost.
