...
**This is a runbook.** The graph lives in [`execution.md`](execution.md); each step is a node file under [`nodes/`](nodes/). Read [`../../runbook/RUNTIME.md`](../../runbook/RUNTIME.md) for walking conventions. The agent is the runtime.
...
1. Parse arguments. Stash mode flags after `sync` initializes state:
   - `.apm/runbook/scripts/runbook-driver --workflow=do set noGit <true|false>`
   - `.apm/runbook/scripts/runbook-driver --workflow=do set minimal <true|false>`
   - `.apm/runbook/scripts/runbook-driver --workflow=do set review <true|false>`
...
3. For each `call <node>` in order:
   - `.apm/runbook/scripts/runbook-driver --workflow=do start <node>`
   - Read `nodes/<node>.md` and follow it.
   - `.apm/runbook/scripts/runbook-driver --workflow=do end <status> "<verification>" [reason]`
4. For a guarded `call` whose guard is false: `.apm/runbook/scripts/runbook-driver --workflow=do skip <node> "<guard text>"`. Skip reasons are the unsatisfied guard expression (e.g. `"not noGit"`, `"not minimal"`, `"forge != github"`).
...
- **Failure**: leave the failing step `in_progress`, mark `done` `completed` after the failure summary is written, run `runbook-driver --workflow=do set status failed`.
