...
A `failed` step always blocks `"completed"`. Update via `runbook-driver --workflow=do set status completed` or `set status failed`.
...
**If `--no-git`**: There is no branch or PR. Print the timing table and optimization suggestions to the terminal only. List the files modified in the working tree (`/vcs op dirty-files` or equivalent) so the user can see what the agent touched. Remind the user that changes are uncommitted.

**If `forge != github`**: Report the branch name (and remote URL via `/vcs op remote-url`) instead of a PR URL. Print the timing table and optimization suggestions to the terminal only — do not attempt to post a PR comment.

**If `forge == github`**: Report the PR URL. Post the final step status table as a PR comment via `/forge op comment-pr`. Use the markdown table and slowest-step line emitted by the runbook done script verbatim (strip the trailing `<<<FACTS ... FACTS` block — that's internal). Format:
