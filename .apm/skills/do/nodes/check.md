...
This is the cheapest gate in the pipeline, so it runs first — fail fast on broken code before any downstream step does work over it. If no check command is documented, skip this step with a note (record via `runbook-driver --workflow=do end skipped "no check command configured" "no check command configured"`).
