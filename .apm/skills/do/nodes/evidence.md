...
- Standard PR context: PR URL, branch name, base branch, current commit SHA (via `/vcs op head-commit-sha`), and `/vcs op diff-names` output so the sub-agent knows which routes/files to exercise.
...
After the sub-agent returns, post its output as one PR comment via `/forge op comment-pr` under a `## Evidence` heading.
...
**Verify**: Either the step was skipped per the rules above, or a `## Evidence` PR comment exists (`/forge op view-pr-comments`) populated from the sub-agent's output.
