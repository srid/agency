...
- Calls `.apm/runbook/scripts/runbook-driver --workflow=do init`.
- Fetches `origin` and pins `origin/HEAD` (via `.apm/skills/vcs/vcs-op fetch`).
...
- Calls `.apm/runbook/scripts/runbook-driver --workflow=do set forge <value>`, `set noGit <value>`, `set vcs <git|jj>`.
- Records the sync step as passed via `.apm/runbook/scripts/runbook-driver --workflow=do step sync passed ...`.
