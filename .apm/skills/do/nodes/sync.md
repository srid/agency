...
- Calls `runbook-driver --workflow=do init`.
- Fetches `origin` and pins `origin/HEAD` (via `/vcs op fetch`).
...
- Calls `runbook-driver --workflow=do set forge <value>`, `set noGit <value>`, `set vcs <git|jj>`.
- Records the sync step as passed via `runbook-driver --workflow=do step sync passed ...`.
