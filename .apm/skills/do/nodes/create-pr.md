...
Check whether a PR already exists for this branch: `.../skills/forge/forge-op view-pr`.

### If no PR exists (first run, normal path)

1. Create a draft PR: `.../skills/forge/forge-op create-pr --draft`
...
2. **Post hickey/lowy/police results** as a PR comment.
   ...
   Post via `.../skills/forge/forge-op comment-pr`.

### If PR already exists (followup runs, `--from` entry points)

Re-check the PR title/body against current scope. If scope changed, update via `.../skills/forge/forge-op edit-pr` per the `forge-pr` skill.
...
**Verify**: Draft PR exists (`.../skills/forge/forge-op view-pr` succeeds), PR title/body matches the delivered scope, hickey/lowy/police findings posted if any.
