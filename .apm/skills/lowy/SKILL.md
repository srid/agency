...
Budget heuristic: this survey is worth its cost when the diff introduces a new top-level module, component, or boundary — typically signalled by new files added (check with `.../skills/do/scripts/vcs-op new-files <base>` when running under `/do`, or `git diff --diff-filter=A --name-only origin/HEAD...HEAD` standalone; jj: tree-comparison). Pure refactors that don't add new boundaries are exempt.
