...
const countMutableState = (fn: Node): { lets: number; reassigns: number } => {
  ...
  fn.forEachDescendant((child) => {
    ...
    if (
      ...
      const op =
        (child as any).getOperatorToken?.() ?? child.getChildAtIndex(0);
...
const checkMutableStateDensity = (
  ...
  sourceFiles.flatMap((sf) => {
    ...
    sf.forEachDescendant((node) => {
      ...
      const isExported = Node.isFunctionDeclaration(node) && node.isExported();
      const isTopLevelVar =
        node.getParent() &&
        ...
        if (Node.isForOfStatement(node) && node.isAwaited()) {
          ...
          if (Node.isCallExpression(iterExpr)) {
            ...
            if (
              ...
              callee.getText() === "subscribeAndYield"
            ) {
...
const summarize = (report: Report): void => {
  ...
  for (const f of report.mutableStateDensity) {
    log(
      `  ${f.file}:${f.line} — ${f.function} (${f.total} mutable ops: ${f.letCount} lets, ${f.reassignmentCount} reassignments)`,
    );
  }
  ...
  for (const f of report.closureOverMutable) {
    log(
      `  ${f.file}:${f.innerLine} — inner fn "${f.innerFunction}" captures \`${f.variable}\` from "${f.outerFunction}" (declared line ${f.outerLine})`,
    );
  }
  ...
  for (const f of report.circularEventFlow) {
    log(
      `  ${f.file}:${f.line} — ${f.function} subscribes and emits "${f.event}"`,
    );
  }
  ...
  for (const f of report.lifecycleNesting) {
    log(
      `  ${f.file}:${f.line} — ${f.generator} (${f.count} lifecycle constructs: ${f.constructs.join(", ")})`,
    );
  }
  ...
  log(
    total === 0
      ? "✓ No structural simplicity issues detected."
      : "✗ Findings above — review for accidental complexity.",
  );
};
