...
- **`hickey`** — Structural simplicity evaluation using [Rich Hickey's "Simple Made Easy"](https://www.infoq.com/presentations/Simple-Made-Easy/) framework. Catches accidental complexity that tests can't. Ships as a sub-agent (`@agent-hickey`) so it can run in parallel with `lowy` without serializing on the main conversation loop. Auto-invoked from `do` (post-implement on the real diff) and `talk` (alongside lowy on the design sketch — prompted to land concrete complecting/fragmentation risks or say explicitly there's nothing yet to bite into).
- **`lowy`** — Volatility-based decomposition review using [Juval Lowy's framework](https://www.informit.com/articles/article.aspx?p=2995357&seqNum=2) (from [*Righting Software*](https://rightingsoftware.org/), building on [Parnas 1972](https://www.win.tue.nl/~wstomv/edu/2ip30/references/criteria_for_modularization.pdf)). Checks that module boundaries encapsulate axes of change, not just functionality. Ships as a sub-agent (`@agent-lowy`). Auto-invoked from both `do` (post-implement on the real diff) and `talk` (on design sketches, alongside hickey).
...
Both default to Sonnet to keep the review cheap enough to run on every task. Pass **`--review-model=opus`** to `do` or `talk` when the diff warrants a deeper pass — large or architecturally significant changes, cross-module refactors, anything you want extra-careful eyes on. `haiku` is also accepted for cheap scans.
...
- [Video talking about adding a /code-police rule](https://youtu.be/IFp0bb2D0ZE?si=1ISdAYeFw5LTaMW1&t=426)
